      ...
      <div className="bg-kid-surface rounded-2xl border border-white/10 w-full max-w-md shadow-2xl">
        ...
        <div className="p-5 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {PLAN_OPTIONS.map(p => (
                <button
                  ...
                  className={`py-2.5 rounded-lg border text-sm font-semibold transition-all ${plan === p.id ? 'bg-kid-accent border-kid-accent text-white' : 'border-white/10 text-gray-300 hover:border-white/30'}`}
                >
                  {p.label}
                </button>
              ))}
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button onClick={handleSubmit} disabled={loading} className="flex-1 bg-kid-accent hover:bg-kid-accent-hover">
...
function EditExpiryModal({ subscription, onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-kid-surface rounded-2xl border border-white/10 w-full max-w-sm shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-white/5">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-kid-panel border border-white/10 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-kid-accent"
            />
          </div>
          <div className="flex gap-3 pt-2">
            <Button variant="outline" onClick={onClose} className="flex-1 border-white/10 text-gray-300">Cancelar</Button>
            <Button onClick={handleSave} disabled={loading} className="flex-1 bg-kid-accent hover:bg-kid-accent-hover">
...
export default function AdminSubscriptions({ embedded = false }) {
  ...
  return (
    <>
    <div className={embedded ? '' : 'min-h-screen bg-kid-page p-6'}>
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-white">Usuários & Assinaturas</h1>
          <div className="flex gap-2">
            <Button onClick={handleGrantTrialAll} disabled={grantingTrial} variant="outline" className="border-kid-sun/40 text-kid-sun hover:bg-kid-sun/10 gap-2">
              {grantingTrial ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Gift className="w-4 h-4" />}
              Trial para todos
            </Button>
            <Button onClick={() => setAddForEmail('')} className="bg-kid-accent hover:bg-kid-accent-hover gap-2">
              <Plus className="w-4 h-4" /> Adicionar Assinatura
            </Button>
          </div>
        </div>
        <div className="mb-6 rounded-xl border border-kid-sun/20 bg-kid-sun/5 px-4 py-3 text-sm text-[#FFE082]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-kid-surface rounded-xl p-4 border border-white/5">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="pl-9 bg-kid-surface border-white/10 text-white placeholder:text-gray-500"
            />
          </div>
          <div className="flex gap-1 bg-kid-surface p-1 rounded-lg border border-white/5">
            {[
              { id: 'all', label: 'Todos' },
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded text-xs font-semibold transition-all ${filter === f.id ? 'bg-kid-accent text-white' : 'text-gray-400 hover:text-white'}`}
              >
                {f.label}
              </button>
            ))}
        ...
        {isLoading ? (
          ...
          <div className="bg-kid-surface rounded-xl border border-white/5 overflow-hidden">
                          ...
                          <div className="flex gap-2 flex-wrap">
                            {!sub || sub.status !== 'active' ? (
                              <Button
                                ...
                                className="bg-kid-accent hover:bg-kid-accent-hover h-7 text-xs gap-1"
                              >
