...
export default function Browse() {
  ...
  const { data: allSeries = [], isLoading } = useLiveEntityList({
    entity: base44.entities.Series,
    query: { published: true },
  });

  const { data: firestoreCategories = [] } = useLiveEntityList({
    entity: base44.entities.Category,
  });
  ...
  const categories = useMemo(() => {
    if (firestoreCategories.length > 0) {
      return ['Todas', ...firestoreCategories.map((item) => item.name)];
    }
    const cats = new Set();
  ...
  }, [byCatalogType, firestoreCategories]);
