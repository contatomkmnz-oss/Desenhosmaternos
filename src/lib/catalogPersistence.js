...
async function postCatalogSnapshotToDisk() {
  try {
    ...
    if (!res.ok && res.status !== 404) {
      console.warn('[KIDSPlay] Autosave para data/catalog-backup.json falhou', res.status);
    }
  } catch (e) {
...
export function downloadCatalogBackupJson() {
  ...
  a.download = `kidsplay-catalog-backup-${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.json`;
  a.click();
