 ...
 * - Se o backup em ficheiro tiver `savedAt` mais recente que `kidsplay_last_catalog_save` → aplica o ficheiro.
...
export async function hydrateCatalogBootstrap() {
  ...
  try {
    ...
    try {
      ...
      console.warn('[KIDSPlay] Ficheiro catalog-backup.json inválido', e);
      return;
    }

    const v = validateCatalogSnapshot(data);
    if (!v.ok) {
      console.warn('[KIDSPlay] Backup em disco ignorado:', v.error);
      return;
    }
    ...
    console.warn('[KIDSPlay] Hidratação a partir de data/catalog-backup.json ignorada', e);
  }
}
